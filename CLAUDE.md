...
- **`plugins/workflow-tools/`** — a plugin declared by `plugin.json`. Currently exposes one agent, `test-baseline` (skill at `plugins/workflow-tools/skills/test-baseline.md`), invoked from `/vuln` and `/upgrade` as `workflow-tools:test-baseline` to capture a pre-change test baseline for regression comparison.
