...
3. Read the relevant command file(s) from `~/.claude/claude-config/commands/`
   (if accessible) to understand the workflow that was used. Focus on the
   section most relevant to the session's events.
4. Scan `~/.claude/claude-config/hooks/` and `~/.claude/claude-config/agents/`
...
#### Reference docs
- **File**: [path, e.g. ~/.claude/claude-config/references/upgrade/compatibility.md]
