...
3. **Parse** — Extract from the output:
   ...
   | Make | best-effort: look for any `X passed` / `X failed` / `X pass` / `X fail` patterns in the combined output | same — Makefile targets shell out to any runner, so patterns are framework-dependent. If no pattern is found, set counts to 0 and include a note explaining that Make wraps an unrecognized runner. |
